...
from tictactoe_utils import (create_state_dictionary, load_qtable, reshape_state,
...
#     max_steps = args.max_steps[0]
    ...
    for eps in [200]*int(train_episodes/200):
        policy, rewards = qLearning(Q,env,gamma, alpha, epsilon, eps)
    ...
    #evaluation
    print('\n***Test results after training({} episodes) against rand opponent***'.format(train_episodes))
...
#     plt.yticks(range(-10,50,2))
    ...
    plt.title('Progress(after every 200 train games against rand opponent)')
#     plt.legend()
    ...
    policy, _ = qLearning(Q,env,gamma, alpha, epsilon, improve_episodes,policy)
#     for eps in [200]*int(train_episodes/200):
